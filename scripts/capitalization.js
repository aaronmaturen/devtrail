  ...
  if (pr.evidence && Array.isArray(pr.evidence)) {
    ...
    for (const item of pr.evidence) {
      if (item.evidence && typeof item.evidence === "string") {
        ...
        const statusMatch = item.evidence.match(
          /Jira ticket.*?status.*?(?:Done|Completed|Resolved|Closed).*?(\d{4}-\d{2}-\d{2})/i
        );
...
function groupPRsByMonth(recentPRs) {
  ...
  Object.entries(recentPRs).forEach(([repo, prs]) => {
    prs.forEach((pr) => {
      ...
      // Format as YYYY-MM
      const monthKey = `${completionDate.getFullYear()}-${String(
...
Only include PRs that represent capitalizable work. If a PR is for bug fixes, maintenance, or other non-capitalizable work, exclude it.`;
    ...
    try {
      console.log(
        chalk.yellow(`Identifying capitalizable features for ${monthName}...`)
      );
      ...
      if (capitalizationTable.includes("|")) {
        const tableLines = capitalizationTable
          .split("\n")
      ...
      console.error(
        chalk.red(
          `Error identifying capitalizable features for ${monthName}: ${error.message}`
        )
      );
  ...
  markdownContent +=
    "- Only capitalizable work (new features, enhancements) is included\n";
  markdownContent +=
    "- Bug fixes, maintenance, and minor tweaks are excluded\n";
