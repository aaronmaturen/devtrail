...
function findLatestComponentAnalysisReport() {
  ...
  const files = fs.readdirSync(REPORTS_DIR)
    .filter(file => file.startsWith('component_analysis_') && file.endsWith('.md'))
    .map(file => ({
      name: file,
...
function extractComponentData(reportPath) {
  ...
  for (const section of componentSections) {
    ...
    if (nameAndRole) {
      ...
      // Extract metrics
      const metricsStart = lines.findIndex(line => line.includes('**Metrics:**'));
      if (metricsStart !== -1) {
        let i = metricsStart + 1;
        while (i < lines.length && lines[i].startsWith('- ')) {
          const metricMatch = lines[i].match(/- (.+): (.+)/);
          if (metricMatch) {
            component.metrics[metricMatch[1].toLowerCase().replace(/ /g, '_')] = metricMatch[2];
          }
          i++;
        }
      }
      ...
      const contributorsStart = lines.findIndex(line => line.includes('**GitHub Contributors:**'));
      if (contributorsStart !== -1) {
        ...
        while (i < lines.length && !lines[i].includes('**Top Contributors:**')) i++;
      ...
      // Extract analysis
      const analysisStart = lines.findIndex(line => line.includes('**Analysis:**'));
      if (analysisStart !== -1) {
      ...
      // Extract recent PRs
      const prsStart = lines.findIndex(line => line.includes('**Recent PRs:**'));
      if (prsStart !== -1) {
        let i = prsStart + 1;
...
async function interactiveReview(grouped, criteria) {
  ...
  const componentData = componentAnalysisPath ? extractComponentData(componentAnalysisPath) : null;
  ...
  console.log(chalk.yellow('• Reflect on performance over the past fiscal year (July 1 – present)'));
  console.log(chalk.yellow('• Review your goals, progress updates, and key project outcomes'));
  console.log(chalk.yellow('• Write 3-5 complete sentences per question'));
  console.log(chalk.yellow('• Include concrete examples, measurable outcomes, and relevant data'));
  console.log(chalk.yellow('• Focus on the impact of your actions on projects, your role, and your team'));
  console.log(chalk.yellow('• Highlight your achievements and the value you bring to the organization'));
  console.log(chalk.dim('─'.repeat(80)));
  
  // Display component analysis data if available
  if (componentData && componentData.length > 0) {
    console.log(chalk.bold.green('\nComponent Analysis:'));
    console.log(chalk.yellow(`Found data for ${componentData.length} components you've contributed to.`));
    ...
    if (leadComponents.length > 0) {
      ...
      leadComponents.forEach(comp => {
        console.log(chalk.white(`• ${comp.name} (${comp.metrics.prs || 0} PRs, ${comp.metrics.total_changes || 0} changes)`));
      });
    }
    
    // Show significant contributor components
    const significantComponents = componentData.filter(c => c.role === 'Significant Contributor');
    ...
    if (componentsWithOtherLeads.length > 0) {
      ...
      componentsWithOtherLeads.forEach(comp => {
        console.log(chalk.white(`• ${comp.name} - Lead: ${comp.githubContributors.lead}`));
      });
    }
  ...
  console.log(chalk.dim('Remember to write 3-5 complete sentences with specific examples and metrics.'));
  const introduction = await askQuestion('How would you describe your overall experience and impact this past fiscal year (July 1 - present)?');
  markdownContent += `${introduction}\n\n`;
  ...
  for (const [area, areaCriteria] of Object.entries(criteriaByArea)) {
    ...
    for (const criterion of areaCriteria) {
      ...
      const assessment = await askQuestion(`How would you rate yourself on "${criterion.subarea}" (1-5)?`);
      console.log(chalk.dim('Remember to include concrete examples, measurable outcomes, and relevant data.'));
      const explanation = await askQuestion('Can you provide specific examples of how you demonstrated this and what impact it had on your projects, role, and team?');
      console.log(chalk.dim('Think about skills you want to develop and how they align with organizational goals.'));
      const growth = await askQuestion('What opportunities do you see for growth in this area?');
  ...
  console.log(chalk.dim('Include metrics, milestones, or historical context to quantify your impact.'));
  const biggestImpact = await askQuestion('What do you believe was your biggest impact on the organization this fiscal year?');
  console.log(chalk.dim('Describe how this learning changed your approach or improved your effectiveness.'));
  const biggestLearning = await askQuestion('What was your most significant learning or growth moment?');
  console.log(chalk.dim('Highlight specific team goals you helped achieve and broader organizational impacts.'));
  const teamContribution = await askQuestion('How did your work contribute to the success of your team and the broader organization?');
  ...
  console.log(chalk.dim('Connect these skills to your career aspirations and organizational needs.'));
  const developmentNeeds = await askQuestion('What specific skills or experiences would help you grow in the coming fiscal year?');
  console.log(chalk.dim('Be specific about resources, training, or opportunities that would help you succeed.'));
  const supportNeeded = await askQuestion('What support do you need from your manager or the organization to achieve your goals?');
  console.log(chalk.dim('Reflect on your proudest accomplishments and their significance.'));
  const proudestAccomplishment = await askQuestion('What accomplishment from this fiscal year are you most proud of and why?');
  ...
  const generateReport = await askQuestion('Would you like to generate a detailed goals progress report based on your PRs? (y/n)').then(answer => answer.toLowerCase().startsWith('y'));
  ...
  if (generateReport) {
    ...
    try {
      ...
      const reportOutput = execSync('node scripts/goals-progress.js', { cwd: path.join(__dirname, '..') }).toString();
      ...
      console.error(chalk.red(`Error generating goals progress report: ${error.message}`));
    }
  }
  
  if (goalsContent) {
    // Ask about goals progress
    const goalsProgress = await askQuestion('How would you rate your overall progress on these goals (1-5)?');
    const goalsComments = await askQuestion('Any comments on your goals progress or adjustments needed?');
    
    markdownContent += '### Goals Progress Assessment\n\n';
    markdownContent += `**Overall Progress Rating:** ${goalsProgress}/5\n\n`;
    markdownContent += `**Comments:**\n${goalsComments}\n\n`;
  } else {
    console.log(chalk.yellow('No goals file found. Skipping goals review section.'));
  }
  ...
  console.log(chalk.bold.green('\nQuestion 1: Key Accomplishments'));
  console.log(chalk.yellow('Reflecting on your focus and goals for FY25, what were your key accomplishments? Provide specific examples of your impact to your team, department or the organization.'));
  console.log(chalk.yellow('Please include your Lattice goals and the extent to which you\'ve achieved them as part of your response.'));
  
  // Generate AI response for Question 1
  console.log(chalk.dim('\nGenerating AI-suggested response...'));
  const accomplishmentsResponse = await generateQuestionResponse('accomplishments', goalsContent, processedPRs, config, componentData);
  ...
  if (accomplishmentsFeedback.toLowerCase() !== 'n') {
    ...
    finalAccomplishmentsResponse = await refineResponse('accomplishments', accomplishmentsResponse, feedbackText, config);
    console.log(chalk.green('\nRefined Response:'));
  ...
  console.log(chalk.bold.green('\nQuestion 2: Areas for Improvement'));
  console.log(chalk.yellow('What are two areas in which you feel you could improve in to increase your impact at Presence?'));
  
  // Generate AI response for Question 2
  console.log(chalk.dim('\nGenerating AI-suggested response...'));
  const improvementResponse = await generateQuestionResponse('improvement', goalsContent, processedPRs, config, componentData);
  console.log(chalk.cyan('\nAI-Suggested Response:'));
  ...
  if (improvementFeedback.toLowerCase() !== 'n') {
    const feedbackText = await askQuestion('Please provide your feedback:');
    console.log(chalk.dim('\nRefining response based on feedback...'));
    finalImprovementResponse = await refineResponse('improvement', improvementResponse, feedbackText, config);
    console.log(chalk.green('\nRefined Response:'));
  ...
  console.log(chalk.bold.green('\nQuestion 3: Future Goals'));
  console.log(chalk.yellow('Please outline your performance and development goals for FY26. How can your manager support you to achieve these goals?'));
  
  // Generate AI response for Question 3
  console.log(chalk.dim('\nGenerating AI-suggested response...'));
  const goalsResponse = await generateQuestionResponse('goals', goalsContent, processedPRs, config);
  console.log(chalk.cyan('\nAI-Suggested Response:'));
  ...
  if (goalsFeedback.toLowerCase() !== 'n') {
    const feedbackText = await askQuestion('Please provide your feedback:');
    console.log(chalk.dim('\nRefining response based on feedback...'));
    finalGoalsResponse = await refineResponse('goals', goalsResponse, feedbackText, config);
    console.log(chalk.green('\nRefined Response:'));
...
async function generateQuestionResponse(questionType, goalsContent, processedPRs, config, componentData = null) {
  try {
    ...
    if (componentData && componentData.length > 0) {
      ...
      if (leadComponents.length > 0) {
        componentSummary += `\n\nYou are the LEAD for ${leadComponents.length} components:\n`;
        leadComponents.forEach(comp => {
          componentSummary += `- ${comp.name} (${comp.metrics.prs || 0} PRs, ${comp.metrics.total_changes || 0} changes)\n`;
          if (comp.analysis) {
      ...
      const componentsWithGitHubData = componentData.filter(c => c.githubContributors && c.githubContributors.lead);
      if (componentsWithGitHubData.length > 0) {
        ...
        componentsWithGitHubData.slice(0, 3).forEach(comp => {
          ...
          if (comp.githubContributors.contributors && comp.githubContributors.contributors.length > 0) {
            componentSummary += `  Top contributors: ${comp.githubContributors.contributors.slice(0, 3).map(c => c.name).join(', ')}\n`;
          }
        });
      }
    }
...
async function refineResponse(questionType, originalResponse, feedback, config) {
  try {
    if (!config.anthropic_api_key) {
