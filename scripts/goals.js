...
async function generateGoals(grouped, config) {
  ...
  const sortedCriteria = Object.values(grouped).sort(
    (a, b) => a.order - b.order
  );

  // Find criteria with no or low evidence
  const criteriaWithNoEvidence = sortedCriteria.filter((c) => c.count === 0);
  const criteriaWithLowEvidence = sortedCriteria.filter(
    (c) => c.count > 0 && c.count < 3
  );
  ...
  const noEvidenceText = criteriaWithNoEvidence
    .map((c) => `${c.id}: [${c.area} > ${c.subarea}] ${c.description}`)
    .join("\n\n");

  const lowEvidenceText = criteriaWithLowEvidence
    .map((c) => {
      const avgConfidence = c.count
        ? Math.round(c.totalConfidence / c.count)
        : 0;
  ...
  if (config.jiraTickets && config.jiraTickets.length > 0) {
    futureTicketsText = config.jiraTickets
      .map((ticket) => {
...
async function fetchFutureJiraTickets(config) {
  if (
    ...
    console.log(
      chalk.yellow("Jira configuration incomplete. Skipping Jira ticket fetch.")
    );
    return [];
  }
